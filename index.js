...
function init(getMail){
    ...
    const query = encodeURIComponent(`Select * where H like "${getMail}"`);
    ...
    .then(rep=>{
        ...
        if(Object.keys(rows).length>0){
            ...
            document.getElementById("name").innerHTML=rows[0].c[1].v
            document.getElementById("date").innerHTML=rows[0].c[3].f
            if(rows[0].c[4].v<=0){
                document.getElementById("days").innerHTML=0
            }else{
                document.getElementById("days").innerHTML=rows[0].c[4].v
            }
            if(rows[0].c[5].v =='Active'){
                document.getElementById("status").innerHTML=`<button type="button" class="btn btn-success btn-sm">${rows[0].c[5].v}</button>`
            }else{
                document.getElementById("status").innerHTML=`<button type="button" class="btn btn-danger btn-sm">${rows[0].c[5].v}</button>`
            }
            // document.getElementById("goto").innerHTML=""
            if(rows[0].c[5].v =='Active'){
                document.getElementById("goto").innerHTML=`<a class="btn btn-success btn-sm" href="${rows[0].c[6].v}" role="button">Go to Canva</a>`
            }else{
